...
internal class TemplateAssemblerImpl(private val stringFormat: StringFormat) : TemplateAssembler {
    override suspend fun assemble(screenSchema: ScreenSchema): ScreenScaffold {
        val root: Identifier = screenSchema.root
        val templates: List<Template> = screenSchema.templates
        val blocks: List<Block> = coroutineScope {
            screenSchema.sections.map { section: Section ->
